...
_TOOL_DEFINITIONS: list[ToolDefinition] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "longitude, lot_size_sqft, building_sqft, computed_bldg_sqft, use_code, "
            "use_description, neighborhood, zoning_class, beds (REAL), baths (REAL), sqft, "
            "year_built, property_type, last_sale_date, last_sale_price, situs_unit, "
            "property_category, ownership_type, record_type, lot_group_key, "
            "parcel_lot_size_sqft, data_notes DATA MODEL "
            ...
            "building_ar_meaning IMPORTANT COLUMN NOTES: - computed_bldg_sqft: PREFERRED "
            "column for property size. Reconciled best-available building sqft that resolves "
            "conflicts between assessor and MLS data. Use this instead of raw sqft or "
            "building_sqft for property evaluation. - data_notes: JSON array of reconciliation "
            "notes explaining how computed_bldg_sqft was derived. - sqft: Raw per-unit living "
            "area from MLS/assessor. For condos and MF 5+, this is ONE unit's sqft. - "
            "building_sqft: Raw total building footprint from assessor (entire structure). - "
            "For single-family homes, sqft ≈ building_sqft. For multi-unit properties, "
            "building_sqft >> sqft. - When computing building density or building-to-lot "
            "ratios, use computed_bldg_sqft (preferred) or building_sqft. - For condos "
            "(record_type='unit'), lot_size_sqft is the SHARED lot for all units — use "
            "lot_group_key to find sibling units and divide lot_size by unit count. property_sales — historical sale "
